...
class drawer:
    ...
    def MovingAround(self, item, color):
        ...
        while self.repeat:
            ...
            self.field = self.getfield.copy()
            self.field[
                0 + posy : objsize[0] + posy, 0 + posx : objsize[1] + posx
            ] = item
            if not self.instanceDrawing:
                time.sleep(0.05)
            if not self.repeat:
...
def prog():
    ...
    # thinking point !
    field = cv2.cvtColor(
        field, cv2.COLOR_BGRA2RGB
    )  # Doesnt do anything since it 0,0,0 color array .. what is should do with zeros !
    ...
    field = cv2.cvtColor(
        field, cv2.COLOR_BGRA2RGB
    )  # here take effect because it has color values already ..

    pin.rectangle(
        field,
        (50, 50),
        (field.shape[1] - 50, field.shape[0] - 50),
        [0.5, 0.5, 0.5],
        10,
        0,
    )
    ...
    while 1:
        ...
        cv2.imshow(
            "numpy player pixels coloring .. show using opencv    @hmae", pin.field
        )
